...
def check_embed(user: dict, lang: str) -> discord.Embed:
    import datetime
    embed = discord.Embed(title=f"👥 {t(lang, 'check', 'title')}", color=0x5865F2)
...
def log_embed(data: dict, lang: str) -> discord.Embed:
    embed = discord.Embed(title=f"✅ {t(lang, 'log', 'title')}", color=0x57F287)
    embed.add_field(name=f"👥 {t(lang, 'log', 'user')}", value=f"<@{data['discord_id']}> ({data['discord_username']})", inline=False)
